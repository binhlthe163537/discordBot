...
async execute(message, args, client) {
    ...
    if (!channel) return attentionembed(message, "Vào channel đi rồi tao phát cho");
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Từ từ tao đang tìm 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#F0EAD6")
            .setDescription(`**:notes: Từ từ tao đang tìm 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
        ...
        if (urlValid) { //send searching link
          ...
          message.channel.send("Thứ nhạc rác rưởi của mày đây")
        }
    ...
    if (urlValid) {
      try {
        songInfo = await ytsr.default.searchOne(search) ;
        song = {
